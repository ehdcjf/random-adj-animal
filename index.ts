...
export function randomAnimal(
	...
	while (true) {
		...
		if (randomAnimal.length > options.maxLength) continue;
		return randomAnimal;
	}
}
...
export function randomAdjective(
	...
	while (true) {
		...
		if (randomAdjective.length > options.maxLength) continue;
		return randomAdjective;
	}
}

export function randomAdjAnimal(
	options: {
		maxLength?: number;
		spacing?: boolean;
	} = {}
	...
	while (true) {
		...
		const size =
			...
			randomAnimal.length +
			(options.spacing ? 1 : 0);
		if (size > options.maxLength) continue;
